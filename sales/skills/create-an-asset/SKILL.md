...
description: 根据您的交易背景生成定制销售资料（落地页、PPT、单页纸、工作流演示）。描述您的潜在客户、受众和目标，获取精美品牌化的可分享资料。触发词：创建销售资料、制作pitch deck、create an asset
---

# 创建销售资料

根据您的潜在客户、受众和目标生成定制销售资料。支持交互式落地页、演示文稿、高管单页纸以及工作流/架构演示。
...
以下情况调用本技能：
- 用户说 `/create-an-asset` 或 `/create-an-asset [CompanyName]`
- 用户要求"创建资料"、"构建演示"、"制作落地页"、"模拟工作流"
- 用户需要为销售对话准备面向客户的交付物
...
本技能通过收集以下背景信息来创建专业销售资料：
- **(a) 潜在客户** — 公司、联系人、对话记录、痛点
- **(b) 受众** — 谁在查看、他们关注什么
- **(c) 目标** — 资料的目的、期望的下一步行动
- **(d) 格式** — 落地页、PPT、单页纸或工作流演示

技能随后进行调研、构建结构并制作精美品牌化的资料，可直接与客户分享。
...
"AI助手 retrieves the Nike contract PDF and extracts the performance
obligations: minimum $50M revenue, 12% margin requirement, quarterly
reporting deadline..."

Step 3: Data Query
"AI助手 formulates a query and sends it to Workato DataGenie:
'Get Q4 2025 revenue and gross margin for Nike brand from Snowflake'..."

Step 4: Results & Synthesis
"Snowflake returns the data. AI助手 compares actuals vs. obligations:
Revenue $52.3M ✓ (exceeded by $2.3M)
Margin 11.2% ⚠️ (0.8% below threshold)..."

Step 5: Insight Delivery
"AI助手 synthesizes findings into an executive summary with
...
- Components: AI助手, Workato DataGenie, Snowflake, PDF contracts

**Output structure:**
```
[Interactive canvas with 5 nodes]
Human → AI助手 → PDF Contracts → Workato → Snowflake
         ↓
